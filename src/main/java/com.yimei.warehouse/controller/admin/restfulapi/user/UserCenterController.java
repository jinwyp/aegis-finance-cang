...
import com.yimei.warehouse.representation.admin.warehouse.EnumAdminWarehouseError;
import com.yimei.warehouse.representation.admin.warehouse.EnumWarehouseAssignType;
...
import com.yimei.warehouse.service.admin.activiti.WorkFlowServiceImpl;
...
public class UserCenterController {
    ...
    public Result onlineTraderManagerClaimTaskMethod(@PathVariable(value = "taskId") String taskId) {
        ...
        if (!StringUtils.isEmpty(task.getAssignee())) {
            if (task.getAssignee().equals(adminSession.getUser().getId())) {
                return Result.error(EnumAdminWarehouseError.你已经处理此任务.toString());
            } else {
                return Result.error(EnumAdminWarehouseError.此任务已经被其他人处理.toString());
            }
        }
        ...
        return Result.error(EnumAdminWarehouseError.你没有权限领取此任务.toString());
    }
    ...
    public Result assignMYROnlineTraderMethod(@PathVariable(value = "taskId") String taskId,
        ...
        if (task == null) return Result.error(EnumAdminWarehouseError.你没有权限处理此任务或者你已经处理过.toString());
        User user = identityService.createUserQuery().userId(userId).singleResult();
        if (user == null) return Result.error(EnumAdminUserError.此用户不存在.toString());
        List<User> userList = new ArrayList<>();
        String financeEventType = "";
        for (EnumWarehouseAssignType type : EnumWarehouseAssignType.values()) {
        ...
        return Result.error(EnumAdminWarehouseError.你没有处理此金融单的权限.toString());
    }
